...
class Section(commands.Cog):
    ...
    async def section_command(self, ctx):
        client = commands.Bot(command_prefix = '.')
        id = client.get_guild(907602893014720522)
        if "✅ valid" in [role.name for role in ctx.author.roles]:
            ...
            if str(ctx.channel) in channels:
                ...
                request.add_field(name='Mode: Section', value=':flag_fr: Vous avez 2 minutes pour rejoindre la partie!')
                request.add_field(name="Mode: Section", value=':flag_gb: You have 2 minutes to join the party!')
                ...
                await ctx.channel.send(f"{ctx.author.mention}, Les parties personnalisées ne peuvent qu'être lancées dans le salon <#907602893782282315> !")
        else: 
