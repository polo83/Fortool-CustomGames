    ...
    async def duo_command(self, ctx):
        ...
        if "✅ valid" in [role.name for role in ctx.author.roles]:
            ...
            if str(ctx.channel) in channels:
                ...
                request.add_field(name='Mode: Solo arène', value=':flag_fr: Vous avez 2 minutes pour rejoindre la partie!')
                request.add_field(name="Mode: Solo arena", value=':flag_gb: You have 2 minutes to join the party!')
                ...
                await ctx.channel.send(f"{ctx.author.mention}, Les parties personnalisées ne peuvent qu'être lancées dans le salon <#912435313371152485> !")
        else: 
